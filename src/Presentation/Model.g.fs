...
module Mutable =
    ...
    type MStableTrafoModel(__initial : Presentation.Model.StableTrafoModel) =
        inherit obj()
        let mutable __current : Aardvark.Base.Incremental.IModRef<Presentation.Model.StableTrafoModel> = Aardvark.Base.Incremental.EqModRef<Presentation.Model.StableTrafoModel>(__initial) :> Aardvark.Base.Incremental.IModRef<Presentation.Model.StableTrafoModel>
        let _offset = ResetMod.Create(__initial.offset)
        
        member x.offset = _offset :> IMod<_>
        
        member x.Current = __current :> IMod<_>
        member x.Update(v : Presentation.Model.StableTrafoModel) =
            if not (System.Object.ReferenceEquals(__current.Value, v)) then
                __current.Value <- v
        ...
        static member Create(__initial : Presentation.Model.StableTrafoModel) : MStableTrafoModel = MStableTrafoModel(__initial)
        static member Update(m : MStableTrafoModel, v : Presentation.Model.StableTrafoModel) = m.Update(v)
        
        override x.ToString() = __current.Value.ToString()
        member x.AsString = sprintf "%A" __current.Value
        interface IUpdatable<Presentation.Model.StableTrafoModel> with
            member x.Update v = x.Update v
    
    
    
    [<CompilationRepresentation(CompilationRepresentationFlags.ModuleSuffix)>]
    module StableTrafoModel =
        [<CompilationRepresentation(CompilationRepresentationFlags.ModuleSuffix)>]
        module Lens =
            let offset =
                { new Lens<Presentation.Model.StableTrafoModel, System.Double>() with
